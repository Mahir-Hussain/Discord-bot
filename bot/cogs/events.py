...
class Events(commands.Cog):
    ...
    async def on_guild_remove(self, guild):
        ...
        await leave_channel.send(
            f"I have been removed from `{guild.name}`, the ID is `{guild.id}`. I am now in `{len(self.bot.guilds)}` guilds."
        )
                ...
                if data:
                    if data.get("guild") == message.guild.id:
                        await message.channel.send(
                            f"{message.author.mention}, I have removed your AFK",
                            delete_after=5,
                        )
    ...
    async def on_afk_ping(self, message):
        ...
        for i in message.mentions:
            ...
            try:
                ...
                if i.id == found:
                    await message.channel.send(
                        f'{message.author.mention}, **{i.display_name}** is currently AFK for: {a["message"]}'
                    )
                    return True
            except Exception:
                return False
